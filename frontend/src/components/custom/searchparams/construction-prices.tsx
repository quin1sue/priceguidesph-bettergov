...
import { ConstructionPriceType, ConstructionItem } from "@/functions/construction-types";
...
type Props = { initialData: ConstructionPriceType };

export default function ConstructionPricesTable({ initialData }: Props) {
  const searchId = useId();
  const categoryId = useId();

  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("All");
  const { visibleKeys, setVisibleKeys } = useColumnVisibility(CONSTRUCTION_COLUMNS);

  if (!initialData.success) {
    return <ErrorState message={initialData.error || "Construction Materials prices could not be loaded."} />;
  }

  const categories = initialData.categories ?? [];

  // Client-side search + category filter (data already paged from API at 200 rows)
  const filteredCategories = categories
    .filter((cat) => selectedCategory === "All" || cat.category === selectedCategory)
    .map((cat) => ({
      ...cat,
      commodities: cat.commodities.map((comm) => ({
        ...comm,
        items: comm.items.filter((item: ConstructionItem) => {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <PageHeader
        ...
        title="Construction Materials Price Guide"
        description="Browse DTI-monitored prevailing prices for Construction Materials in the National Capital Region. Filter by report date, category, or search by product name."
      />
      <DataDetails
        source={{ name: initialData.source || "DTI NCR", href: "#" }}
        date={initialData.date}
      >
        Prevailing prices for Construction Materials are monitored in the NCR.
      </DataDetails>
      ...
      <section aria-label="Data filters" className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
          <form className="flex flex-col gap-4 sm:flex-row sm:items-end sm:flex-wrap" onSubmit={(e) => e.preventDefault()}>
            ...
            <label className="grid gap-1.5 text-sm font-medium text-slate-800" htmlFor={searchId}>
              ...
              <input
                ...
                placeholder="Product, brand, size…"
          ...
          <div className="flex items-end">
            <FilterFields columns={CONSTRUCTION_COLUMNS} visibleKeys={visibleKeys} onChange={setVisibleKeys} />
          </div>
        </div>
      </section>
      ...
      {filteredCategories.length ? (
        <section aria-live="polite" className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm whitespace-nowrap">
              <caption className="sr-only">
                Construction Materials prevailing prices in Philippine pesos — NCR, {initialData.reportDate}
              </caption>
              <thead className="border-b border-slate-200 bg-slate-50 text-left text-slate-700">
                <tr>
                  ...
                  {show("commodity") && <th scope="col" className="px-4 py-3 font-semibold min-w-[220px]">Commodity</th>}
                  {show("brand") && <th scope="col" className="px-4 py-3 font-semibold min-w-[100px]">Brand</th>}
                  {show("unit") && <th scope="col" className="px-4 py-3 font-semibold min-w-[80px]">Unit</th>}
                  {show("size") && <th scope="col" className="px-4 py-3 font-semibold min-w-[100px]">Size</th>}
                  {show("current") && <th scope="col" className="px-4 py-3 font-semibold text-right min-w-[110px]">Current Price</th>}
                  {show("prev") && <th scope="col" className="px-4 py-3 font-semibold text-right min-w-[110px]">A Month Ago</th>}
                  ...
                  {show("3ago") && <th scope="col" className="px-4 py-3 font-semibold text-right min-w-[120px]">3 Months Ago</th>}
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredCategories.map((cat) =>
                  cat.commodities.map((comm) =>
                    comm.items.map((item: ConstructionItem, idx) => (
                      <tr key={item.id || idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        {show("unit") && <td className="px-4 py-2 text-slate-600">{item.unit || "-"}</td>}
                        {show("size") && <td className="px-4 py-2 text-slate-600">{item.size || "-"}</td>}
                        ...
                        {show("mPhp") && <td className={`px-4 py-2 text-right ${pctClass(item.month_change_php)}`}>{fmt(item.month_change_php)}</td>}
                        ...
                        {show("3php") && <td className={`px-4 py-2 text-right ${pctClass(item.three_month_change_php)}`}>{fmt(item.three_month_change_php)}</td>}
                      </tr>
                    ))
                  )
                )}
          ...
          {initialData.pagination && initialData.pagination.pages > 1 && (
            <footer className="border-t border-slate-200 px-4 py-3 text-sm text-slate-600">
              Page {initialData.pagination.page} of {initialData.pagination.pages} —{" "}
              {initialData.pagination.total} total records
            </footer>
          )}
        </section>
      ) : (
        <EmptyState
          ...
          description={search ? `No Construction Materials match "${search}". Try a different search term.` : "Try choosing all categories."}
        />
      )}
    </main>
  );
